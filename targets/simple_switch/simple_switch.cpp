...
uint64_t tsn_monotonic_us() {
  return tsn_monotonic_ns() / 1000ull;
}

void write_tsn_phase_file(uint64_t mono_ns,
                          ...
                          uint64_t slot_us,
                          uint64_t cycle_us) {
  const char *path_env = std::getenv("TSN_PHASE_PATH");
  const char *path = path_env ? path_env : "/tmp/tsn_phase.txt";
  ...
  out << "mono_ns=" << mono_ns << "\n"
      << "bmv2_now_us=" << scheduler_now_us << "\n"
      << "phase_us=" << phase_us << "\n"
      << "slot_offset_us=" << slot_offset_us << "\n"
      << "slot_id=" << slot_id << "\n"
      << "slot_us=" << slot_us << "\n"
      << "cycle_us=" << cycle_us << "\n";
  out.close();
...
SimpleSwitch::egress_thread(size_t worker_id) {
  ...
  while (1) {
    ...
    static const std::array<int, SSWITCH_PRIORITY_QUEUEING_NB_QUEUES> TSN_GCL = []() {
        ...
        std::ifstream fin(gcl_path);

        if (!fin.good()) {
            bm::Logger::get()->warn(
                "TSN_GCL config file {} not found, using default GCL", gcl_path);
            return gcl;
        }
        ...
        if (values.size() != SSWITCH_PRIORITY_QUEUEING_NB_QUEUES) {
            bm::Logger::get()->error(
                "TSN_GCL config must contain {} values, got {}, using default GCL",
                SSWITCH_PRIORITY_QUEUEING_NB_QUEUES, values.size());
            return gcl;
        }
        ...
        for (size_t i = 0; i < values.size(); i++) {
            if (values[i] != -1 &&
                (values[i] < 0 ||
                    values[i] >= static_cast<int>(SSWITCH_PRIORITY_QUEUEING_NB_QUEUES))) {
                bm::Logger::get()->error(
                    "TSN_GCL invalid queue index at slot {}: {}, using default GCL",
                    i, values[i]);
                return gcl;
            }
        }
        ...
        for (size_t i = 0; i < gcl.size(); i++) {
            bm::Logger::get()->info(
                "TSN_GCL config slot_id={} target_queue_idx={}", i, gcl[i]);
        }
    ...
    while (true) {
      ...
      uint64_t last = last_phase_write_us.load();

      if (now_us >= last + 5000 &&
          last_phase_write_us.compare_exchange_strong(last, now_us)) {
        write_tsn_phase_file(
            mono_ns,
            now_us,
            phase_us,
            slot_offset_us,
      ...
      if (popped) {
        bm::Logger::get()->info(
            "TSN_GCL dequeue now_us={} phase_us={} slot_offset_us={} "
            "worker_id={} slot_id={} target_queue_idx={} "
...
#ifdef SSWITCH_PRIORITY_QUEUEING_ON
        qid_f.set(SSWITCH_PRIORITY_QUEUEING_NB_QUEUES - 1 - queue_idx);
#else
        qid_f.set(0);
#endif
      }
