#!/usr/bin/env python3
"""topo_from_json 后台变体：支持 --duration N 自动运行/退出，供全自动测试使用。"""
...
def register_named_netns(nodes, host_names):
    os.makedirs("/var/run/netns", exist_ok=True)
    for host_name in host_names:
        link_path = f"/var/run/netns/{host_name}"
        if os.path.lexists(link_path):
            os.unlink(link_path)
        os.symlink(f"/proc/{nodes[host_name].pid}/ns/net", link_path)
        info(f"*** netns {host_name}: {link_path} -> /proc/{nodes[host_name].pid}/ns/net\n")


def unregister_named_netns(host_names):
    for host_name in host_names:
        link_path = f"/var/run/netns/{host_name}"
        if os.path.lexists(link_path):
            os.unlink(link_path)
...
def main():
    ...
    parser.add_argument(
        ...
        type=float,
        default=0,
        help="Auto-run duration in seconds, 0 = interactive CLI",
    )
    ...
    net.start()
    net.staticArp()
    register_named_netns(nodes, cfg["hosts"].keys())
    ...
    try:
        ...
        # ====== Background mode vs interactive CLI ======
        if args.duration > 0:
            info(f"\n*** [AUTO] Running for {args.duration:.0f}s (background mode)\n")
            info("*** [AUTO] Topology is alive - run traffic scripts now.\n")
            info("*** [AUTO] Use: sudo ip netns exec <host> <cmd>\n")
            try:
                time.sleep(args.duration)
            except KeyboardInterrupt:
                info("\n*** [AUTO] Interrupted, stopping...\n")
        else:
            CLI(net)
    finally:
        unregister_named_netns(cfg["hosts"].keys())
        net.stop()
