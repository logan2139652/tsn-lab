...
CONFIG="${1:-$HOME/tsn-lab/configs/cbqf_6sw_4host.json}"
...
release_delay_slots = 0
if cfg.get("mechanism", "").lower() == "cbqf" or cfg.get("cbqf", {}).get("enabled"):
    release_delay_slots = int(cfg.get("cbqf", {}).get("release_delay_slots", 1))
...
for flow in session_cfg.get("flows", []):
    ...
    batch_size = int(flow.get("batch_size", session_cfg.get("batch_size", 1)))
    packets = int(duration * 1e6 / cycle_us * len(slots) * batch_size)
...
for sw_name, sw_routes in routes.items():
    for r in sw_routes:
        if r.get("src_mac", "") == src_mac and r.get("dst_mac", "") == dst_mac:
            hops += 1
            break
# CBQF deliberately delays TSN batches by release_delay_slots.
threshold_us = (hops + 2 + release_delay_slots) * slot_us
...
# CBQF batch completeness
step "CBQF Batch Completeness"
echo "SESSION    FID   BATCH_SIZE   BATCHES   COMPLETE   INCOMPLETE   BATCH_P95(us)   BATCH_MAX(us)"
echo "--------------------------------------------------------------------------------------------"
for csv in "$RESULT_DIR"/tsn_recv_*.csv; do
    [[ -f "$csv" ]] || continue
    session=$(basename "$csv" | sed 's/tsn_recv_//;s/\.csv//')
...
with open(csv_path) as f:
    ...
    for row in reader:
        ...
        batch_no = int(row.get("batch_no") or (int(row.get("seq", 0)) // max(1, batch_size)))
        key = (fid, batch_no, batch_size)
        groups.setdefault(key, []).append(float(row["delay_us"]))

by_fid = {}
for (fid, batch_no, batch_size), delays in groups.items():
    by_fid.setdefault((fid, batch_size), []).append(delays)

for (fid, batch_size), batches in sorted(by_fid.items()):
    complete = sum(1 for ds in batches if len(ds) >= batch_size)
    incomplete = len(batches) - complete
    batch_delays = [max(ds) for ds in batches if ds]
    batch_delays.sort()
    if batch_delays:
        p95 = batch_delays[int((len(batch_delays)-1)*0.95)]
        mx = max(batch_delays)
    else:
        p95 = mx = 0.0
    print(f"{sess:8s}  {fid:>4d}  {batch_size:>10d}  {len(batches):>8d}  "
          f"{complete:>8d}  {incomplete:>10d}  {p95:>13.0f}  {mx:>13.0f}")
PYEOF
done
...
for sw in s1 s2 s3 s4 s5 s6; do
    ...
    if [[ -f "$log" ]]; then
        cp "$log" "$RESULT_DIR/"
        enq=$(grep -c 'CBQF_ENQUEUE\|TQF_ENQUEUE\|TCQF_ENQUEUE\|CSQF_ENQUEUE\|TSN_QUEUE.*enqueue' "$log" 2>/dev/null || echo 0)
        deq=$(grep -c 'CBQF_DEQUEUE\|TQF_DEQUEUE\|TCQF_DEQUEUE\|CSQF_DEQUEUE\|TSN_GCL.*dequeue\|CSQF dequeue' "$log" 2>/dev/null || echo 0)
        printf "%-6s  %8s  %8s\n" "$sw" "$enq" "$deq"
    else
        printf "%-6s  %8s  %8s\n" "$sw" "-" "-"
    fi
