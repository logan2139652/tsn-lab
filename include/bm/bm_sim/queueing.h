...
class QueueingLogicPriRL {
 ...
 public:
  ...
  // only try to dequeue form the specified priority queue
  bool try_pop_back_priority(size_t worker_id, size_t target_priority,
                             size_t *queue_id, size_t *priority,
                             T *pItem) {
    LockType lock(mutex);
    auto &w_info = workers_info.at(worker_id);

    if (target_priority >= nb_priorities) return false;
    if (w_info.size == 0) return false;

    auto &queue = w_info.queues[target_priority];
    if (queue.size() == 0) return false;

    auto now = clock::now();
    if (queue.top().send > now) return false;
